...
void vm_mem_compact(VirtualMachine* vm) {
    ...
    for(uint32_t idx = 0; idx < vm->chunk_count; idx++) {
        ...
        if(vm->chunks[idx].index > last_chunk_end) {
            ...
            // previous chunk
            for(uint32_t j = vm->chunks[idx].index; j < vm->chunks[idx].index + vm->chunks[idx].size; j++) {
            ...
            for(uint32_t i = 0; i < vm->stack_frame_count; i++) {
                for(uint32_t j = 0; j < REGISTER_COUNT + 1; j++) {
                    if(vm->stack_frames[i].registers[j].is_ref && vm->stack_frames[i].registers[j].value >= vm->chunks[idx].index && vm->stack_frames[i].registers[j].value < vm->chunks[idx].index + vm->chunks[idx].size) {
                        vm->stack_frames[i].registers[j].value = last_chunk_end + (vm->stack_frames[i].registers[j].value - vm->chunks[idx].index);
                    }
                }
            }
...
int vm_request_mem(VirtualMachine* vm, uint32_t size) {
    if(!size)
        error_out("attempted to allocate a zero sized chunk") // can't allocate zero bytes
    ...
    if(!inserted)
        fresh_chunks[vm->chunk_count] = new_chunk;
...
int vm_free_mem(VirtualMachine* vm, uint32_t addr) {
    uint32_t found = 0;
    for(uint32_t i = 0; i < vm->chunk_count; i++)
        if((found = vm->chunks[i].index == addr))
            break;
    if(!found)
...
#define get_label(uid, vm, lid)                     \
    int label_pos_##uid = -1;                       \
...
int execute_program(VirtualMachine* vm, uint32_t* program, uint32_t length) {
    ...
    for(uint32_t i = 5; i < length; i++) {
        ...
        uint32_t args[2] = {program[((i + 1) < length) * (i + 1)], program[((i + 2) < length) * (i + 2)]};
        i += 1 + !!(opcode & TWO_FLAG);
        ...
        switch(opcode) {
            ...
            case REQ: {
                int mem_request = vm_request_mem(vm, vm->stack_frames[0].registers[args[0]].value);
                vm->stack_frames[0].registers[args[1]].value = mem_request;
                vm->stack_frames[0].registers[args[1]].is_ref = 1;

                // if the allocation failed, try compacting and allocating again
                if(mem_request == -1) {
                    vm_mem_compact(vm);
                    int new_mem_request = vm_request_mem(vm, vm->stack_frames[0].registers[args[0]].value);
                    vm->stack_frames[0].registers[args[1]].value = new_mem_request;
                    vm->stack_frames[0].registers[args[1]].is_ref = 1;

                    // if it still failed, than the heap just doesn't have enough free space
                    if(new_mem_request == -1)
